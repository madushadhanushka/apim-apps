...
describe("Create GraphQl API from file", () => {
    ...
    const apiContext=`/swapi${Math.floor(Math.random() * (100000 - 1 + 1) + 1)}`;
    const apiName=`StarWarsAPIGQL${Math.floor(Math.random() * (100000 - 1 + 1) + 1)}`;
    ...
    it("Verify GraphQl API Capabilities", () => {
        ...
        .then((href) => {
          ...
          let adminScope = `adminScope${Math.floor(Math.random() * (100000 - 1 + 1) + 1)}`
        ...
        .then((href) => {
          let filmSubscriberScope = `filmSubscriberScope${Math.floor(Math.random() * (100000 - 1 + 1) + 1)}`
          cy.visit(`${Utils.getAppOrigin()}${href}/create`);
